...
# Load model and tokenizer once (Global Variables)
...
def errant_annotate_corrections(text, corrections):
    ...
    orig = annotator.parse(text, tokenise=True)
    annotations = []
    for corr_text in corrections:
        corr_text = preprocess_text(corr_text)
        cor = annotator.parse(corr_text, tokenise=True)
        annotations.append(annotator.annotate(orig, cor))
    return annotations
...
def grammatical_error_correction_with_props(input_sentence, num_return_sequences=5):
    ...
    for i, output in enumerate(outputs.sequences):
        ...
        probability = math.exp(log_prob)  # Convert log_prob to probability
        corrected_sentences.append({"generated_text": corrected_text, "log_prob": log_prob, "probability": probability})

    return corrected_sentences


def simple_error_detection(annotations):
    return [{"span": (e.o_start, e.o_end), "type": e.type} for e in annotations[0]]


def multi_union_error_detection(annotation_list, verbose=False):
    span_error_tree = {}
    for i, error_annotations in enumerate(annotation_list):
        for e in error_annotations:
            o_span = (e.o_start, e.o_end)
            if o_span not in span_error_tree:
                span_error_tree[o_span] = []
            span_error_tree[o_span].append(e.type)

    if verbose:
        print("span_error_tree", span_error_tree)

    # return {key: Counter(values).most_common(1)[0][0] for key, values in span_error_tree.items()}

    return [{"span": span, "type": Counter(errors).most_common(1)[0][0]} for span, errors in span_error_tree.items()]


def multi_error_majority_error_detection(annotation_list, verbose=True):
    min_occurrences = (len(annotation_list) // 2) + 1
    span_error_tree = {}
    for i, error_annotations in enumerate(annotation_list):
        for e in error_annotations:
            o_span = (e.o_start, e.o_end)
            if o_span not in span_error_tree:
                span_error_tree[o_span] = {}
            if e.type not in span_error_tree[o_span]:
                span_error_tree[o_span][e.type] = []
            span_error_tree[o_span][e.type].append((i, e))

    res = []
    for span, error_dict in span_error_tree.items():
        error_counts = {error: len(occurrences) for error, occurrences in error_dict.items()}
        most_common_error = max(error_counts, key=error_counts.get)
        if error_counts[most_common_error] >= min_occurrences:
            res.append({
                "span": span,
...
def multi_span_majority_error_detection(annotation_list, verbose=True):
    min_occurrences = (len(annotation_list) // 2) + 1
    span_error_tree = {}
    for i, error_annotations in enumerate(annotation_list):
        for e in error_annotations:
    ...
    res = []
    for span, error_dict in span_error_tree.items():
        e_sum = sum(len(errors) for errors in error_dict.values())
        if e_sum >= min_occurrences:
            most_common_error = max(error_dict, key=lambda k: len(error_dict[k]))
            res.append({
                "span": span,
                "type": most_common_error,
                "total_occurrences": e_sum,
                "most_common_count": len(error_dict[most_common_error]),
                "min_occurrences": min_occurrences
            })
    return res


def multi_prob_error_detection(annotations_list, probs):
    assert len(annotations_list) == len(probs)
    error_path_probs = {}
    min_cumulative_prob = max(probs)
    for i, (error_annotations, prob) in enumerate(zip(annotations_list, probs)):
        for e in error_annotations:
            o_span = (e.o_start, e.o_end)
            if o_span not in error_path_probs:
                error_path_probs[o_span] = {}
            if e.type not in error_path_probs[o_span]:
                error_path_probs[o_span][e.type] = 0.0
            error_path_probs[o_span][e.type] += prob

    res = []
    for span, error_dict in error_path_probs.items():
        most_probable_error = max(error_dict, key=error_dict.get)  # Most probable error type
        cumulative_prob = error_dict[most_probable_error]
        if cumulative_prob >= min_cumulative_prob:
            res.append({
                "span": span,
                "type": most_probable_error,
                "cumulative_prob": cumulative_prob,
                "min_cumulative_prob": min_cumulative_prob
            })
...
class EvaluationPipeline:
    ...
    def __init__(self, config: EvalConfig, env: str = "production"):
        ...
        # Setup logging
        self.logging_path = str(os.path.join(self.config.alt_base_path, 'logs', "runs", config.group, config.job_type,
    ...
    def load_data(self, test_corpora: List[CorpusConfig]) -> Dataset:
        ...
        for corpus_config in test_corpora:
            ...
            ds = ds.select_columns(
                [col for col in ["audio_id", "audio", self.label_feature, self.raw_label_feature, "duration"] if
                 col in ds.column_names])
    ...
    def create_tokenizer(self, unk_token: str = "[UNK]", pad_token: str = "[PAD]",
                         word_delimiter_token: str = "|") -> Wav2Vec2CTCTokenizer:
        """
        ...
        """
        self.logger.log_event("Create Tokenizer", unk_token=unk_token, pad_token=pad_token,
                              word_delimiter_token=word_delimiter_token)
        assert os.path.exists(self.vocab_file), f"Vocabulary file not found at {self.vocab_file}"

        decoder_tokenizer = Wav2Vec2CTCTokenizer(self.vocab_file, unk_token=unk_token, pad_token=pad_token,
                                                 word_delimiter_token=word_delimiter_token)
    ...
    def create_processor(self, feature_size: int = 1, sampling_rate: int = 16_000,
                         lm_path: str = None) -> Wav2Vec2Processor:
        """
    ...
    def evaluate(self, dataset, processor, model):
        ...
        if config.wepr:
            ...
            annot_eval = AnnotationEvaluation(references=results["raw_label_str"],
                                              list_of_predictions=[results["pred_str"]], symbols=["@!"],
                                              num_threads=5)
    ...
    def errant_analysis(self, rows: Dataset, text_key: str = "pred_str", test_text_key: str = "raw_label_str"):
        ...
        results = {
            name: {
                "hits_s": [], "misses_s": [], "unnecessary_s": [], "hits_e": [], "misses_e": [], "unnecessary_e": []
