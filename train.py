...
class Wav2VecPipeline:
    ...
    def __init__(self, config: TrainConfig, env: str = "production"):
        ...
        # Setup logging
        self.logging_path = str(os.path.join(self.config.alt_base_path, 'logs', "runs", config.group, config.job_type, config.experiment_label or ""))
    ...
    def compute_metrics(self, pred):
        ...
        label_strs = self.tokenizer.batch_decode(pred.label_ids, group_tokens=False, skip_special_tokens=True)  # we do not want to group tokens when computing the metrics
